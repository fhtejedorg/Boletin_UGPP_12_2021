...
tabla3_variaciones$RangoIBC <- factor(tabla3_variaciones$RangoIBC, levels = unique(tabla3$total$RangoIBC))

salida_table3_variaciones <- tabla3_variaciones %>% arrange(Tipologia, RangoIBC) %>%
  gt(groupname_col = 'Tipologia', rowname_col = 'RangoIBC') %>% 
  data_color(
    columns = 3:12, 
    colors = scales::col_numeric(
